...
const { IsNull, Between, LessThanOrEqual, MoreThanOrEqual, Like, ILike } = require('typeorm');
...
const TransactionRepository = AppDataSource.getRepository('Transaction').extend({
  ...
  findByUserPaginated(userId, filters, skip, take) {
    ...
    if (filters.date_from) qb.andWhere('t.date >= :dateFrom', { dateFrom: filters.date_from });
    if (filters.date_to) qb.andWhere('t.date <= :dateTo', { dateTo: filters.date_to });
